...
> **Later note — the limit law depends on the parity of $n$.** Working it out gives
> $$n\,\widehat{\mathrm{Var}} \;\xrightarrow{d}\; \frac{1}{4f^2(\theta)}\times\begin{cases}[\chi^2_2/2]^2 & n \text{ even (mean 2, variance 20)}\\ [\chi^2_4/4]^2 & n \text{ odd (mean 1.5, variance 5.25)}\end{cases}$$
> because with $n$ even the leave-one-out medians take two values and **one** spacing enters, while with $n$ odd there is a middle observation that can itself be deleted, they take three values, and **two** spacings enter and get averaged. The law printed here is the even one, stated in a section that assumes $n = 2m-1$ odd; it is correct, and what it omits is that the parity decides. The inconsistency, which is the point, is the same either way. Both cases are derived and simulated in the [implementation](../implementations/1979-efron-bootstrap/).
