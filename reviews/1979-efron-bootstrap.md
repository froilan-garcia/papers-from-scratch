...
- **Median (Sec. 3):** the bootstrap correctly estimates (asymptotically) the variance $n E_*(R^*)^2 \to 1/4f^2(\theta)$, **a case in which the jackknife is inconsistent** ($n\,\mathrm{Var}(R) \to \tfrac{1}{4f^2}\cdot[\chi_2^2/2]^2$, with mean 2 and variance 20). It is the flagship argument for the bootstrap. *(Later note: that limit law holds for **even** $n$, while this section assumes $n = 2m-1$ odd. With $n$ even the leave-one-out medians take two values and one spacing enters, giving $[\chi^2_2/2]^2$ exactly as printed; with $n$ odd there is a middle observation that can itself be deleted, they take three values, two spacings enter, and the limit is $\tfrac{1}{4f^2}[\chi_4^2/4]^2$ of mean 1.5 and variance 5.25. Both parities are simulated in the [implementation](../implementations/1979-efron-bootstrap/). The inconsistency, which is the point, is the same either way.)*
