...
> **Later note — the limit law depends on the parity of $n$.** With $n$ **even** the leave-one-out medians take two values and **one** spacing enters, giving $n\,\widehat{\mathrm{Var}} \to \tfrac{1}{4f^2(\theta)}\,[\chi^2_2/2]^2$, of mean 2 and variance 20 — the law printed here. With $n$ **odd** there is a middle observation that can itself be deleted, they take three values, and **two** spacings enter and get averaged, giving $n\,\widehat{\mathrm{Var}} \to \tfrac{1}{4f^2(\theta)}\,[\chi^2_4/4]^2$, of mean 1.5 and variance 5.25. So the printed law is correct, but it is stated in a section that assumes $n = 2m-1$ odd, and what it omits is that the parity decides which of the two applies. The inconsistency, which is the point, is the same either way. Both cases are derived and simulated in the [implementation](../implementations/1979-efron-bootstrap/).
